...
class ForwardKinematics(object):
     ...
     * joint.axis: (only if
     type is 'revolute') the axis of rotation of the joint
    ...
    def compute_transforms(self, link_names, joints, joint_values):
        ...
        # YOUR CODE GOES HERE
        parent = 'world_link'
        rospy.loginfo(parent)
        for i, link_name in enumerate(link_names):
            rospy.loginfo('link_name=' + link_name)
            joint = joints[i]           
            joint_value = 0.0
            for j, v in enumerate(joint_values.name):                
                if joint.name == v:               	    
                    joint_value = joint_values.position[j]                    
                    break
            
            r = joint.origin.rpy               
            t = joint.origin.xyz
            q1 = tf.transformations.quaternion_from_euler(r[0], r[1], r[2])               
            Ts = tf.transformations.translation_matrix((t[0], t[1], t[2]))
            R = tf.transformations.quaternion_matrix(q1)
            M_b_o = tf.transformations.concatenate_matrices(Ts, R)  
            T = tf.transformations.concatenate_matrices(T, M_b_o)           
            if joint.type == 'revolute':                 
                q2 = tf.transformations.quaternion_about_axis(joint_value,joint.axis)
                R2 = tf.transformations.quaternion_matrix(q2)                
                T = tf.transformations.concatenate_matrices(T, R2)                                   
                
            child = link_name
            rospy.loginfo('child: {}, joint_value: {}'.format(child, joint_value))
