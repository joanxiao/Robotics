...
def cartesian_control(joint_transforms, b_T_ee_current, b_T_ee_desired,
    ...
    current_T_desired = numpy.dot(tf.transformations.inverse_matrix(b_T_ee_current), b_T_ee_desired)
    #print(current_T_desired)
    delta_x = numpy.zeros(6)
    delta_x[:3] = tf.transformations.translation_from_matrix(current_T_desired) 
    #print('delta_x[:3]={}'.format(delta_x[:3]))
    angle, axis = rotation_from_matrix(current_T_desired)  
    #print('angle={}'.format(angle))
    #print('axis={}'.format(axis))
    delta_x[3:] = angle * axis
                       
    p = 15.0
    x_dot = p * delta_x
    #print('x_dot={}'.format(x_dot))
    #safe guard
    norm_x_dot = numpy.linalg.norm(x_dot)
    if norm_x_dot != 0 and norm_x_dot > 1:
    	x_dot *= 1 / norm_x_dot                               
    #print('norm of x_dot={}'.format(numpy.linalg.norm(x_dot)))
    #print('b_T_ee_current.shape={}'.format(b_T_ee_current.shape))
    #V_ee = numpy.dot(b_T_ee_current, x_dot)
    V_ee = x_dot
    ...
    for j in range(num_joints):
        ...
        #print('j_T_ee.shape={}'.format(j_T_ee.shape))
        #print(j_T_ee)
        j_R_ee = j_T_ee[:3, :3] #tf.transformations.rotation_from_matrix(j_T_ee)
        #print(j_R_ee)
        #print('j_R_ee.shape={}'.format(j_R_ee.shape))
                                  
        j_t_ee = tf.transformations.translation_from_matrix(j_T_ee)                        
        ee_R_j = tf.transformations.inverse_matrix(j_R_ee)
                                  
        Vj1 = numpy.vstack((ee_R_j, numpy.zeros((3,3))))
        Vj2 = numpy.vstack((-1.0 * numpy.dot(ee_R_j, S_matrix(j_t_ee)), ee_R_j))                 
        Vj = numpy.hstack((Vj1, Vj2))
        
        J[:, j:(j+1)] = Vj[:, -1].reshape((6,1))                            
                           
    J_pinv = numpy.linalg.pinv(J) 
    dq = numpy.dot(J_pinv, V_ee) 
    #safe guard
    norm_dq = numpy.linalg.norm(dq)
    if norm_dq != 0 and norm_dq > 1:
    	dq *= 1 / norm_dq  
...
class CartesianControl(object):
    ...
    def align_with_z(self, axis):
        ...
        if dot == 1: return T
        if dot == -1: return tf.transformations.rotation_matrix(math.pi, x)
