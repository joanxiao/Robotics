...
class MoveArm(object):
    ...
    def get_sample_point(self, q_min, q_max):
        point = []
        for i in range(len(q_min)):
            point.append(random.uniform(q_min[i], q_max[i]))
    ...
    def get_closest_node(self, nodes, point):
        #print('get_closest_node')
        #nodes = []
        #stack = [node]
        #while stack:
        #    cur_node = stack[0]
        ...
        #        stack.append(child)
        #print('number of nodes in tree: {}'.format(len(nodes)))
        
        min_dist = sys.float_info.max
        closest_node = None
        for node in nodes:
            d = self.distance_squared(node.value, point)
            if d < min_dist:
                min_dist = d
                closest_node = node    
    ...
    def is_collision_free(self, point1, point2):
        #print('is_collision_free: point1={} point2={}'.format(point1, point2))
        v1 = numpy.asarray(point1)
        v2 = numpy.asarray(point2)
        delta = (v2 - v1) / numpy.linalg.norm(v2 - v1)        
        delta *= 0.05        
        v_check_point = v1 + delta                  
        
        getting_closer = True       
        while getting_closer:
            check_point = v_check_point.tolist()
            distance = numpy.sqrt(numpy.sum((v2 - v_check_point)**2))            
            if not self.is_state_valid(check_point):
             	#print('collision c={}  v_check_point={}'.format(c,  v_check_point))
                return False
            new_v = v_check_point.copy() + delta
            v_check_point = new_v
            curr_distance = numpy.sqrt(numpy.sum((v2 - v_check_point)**2))
            if curr_distance >= distance:
            	getting_closer = False            
        
        #print('is_collision_free: True v_check_point={} v2={} '.format(v_check_point, v2))
        return True
    ...
    def motion_plan(self, q_start, q_goal, q_min, q_max):
        ...
        q_list = [q_goal]
        node = TreeNode(q_start)        
        nodes = [node]
        
        count = 0
        goal_node = None
        while count < 800:
           point = self.get_sample_point(q_min, q_max)
           #print('sample point: {}'.format(point))                     
        
           if len(node.children) == 0:
               closest_node = node               
           else:               
               closest_node = self.get_closest_node(nodes, point)              
               
           v1 = numpy.asarray(closest_node.value)
           v2 = numpy.asarray(point)
           delta = (v2 - v1) / numpy.linalg.norm(v2 - v1)        
           delta *= 0.5
           point = (v1 + delta).tolist()         
           
           if self.is_collision_free(closest_node.value, point):
               new_node = TreeNode(point)
               #print('collision free from closest_node to point')
               closest_node.add_child(new_node)
               nodes.append(new_node)
               
               if self.is_collision_free(new_node.value, q_goal):
                   #print('goal collision free')
                   goal_node = TreeNode(q_goal)
                   new_node.add_child(goal_node)
                   
                   break
               #else:
                   #print('collision from point to goal')
               
           #else:
               #print('collision from closest_node to point')
       	   count += 1
        ...
        if goal_node is not None:
           ...
           while node is not None:
               #print('insert into path: node.value={}'.format(node.value)) 
               q_list.insert(0, node.value)
               node = node.parent
               
        #trim the path                   
        print('path length={}'.format(len(q_list)))
        ...
        for i in range(1, l-1):
            if last_deleted == -1:
                if self.is_collision_free(q_list[i-1], q_list[i+1]):                     
                    #del q_list[i]                  
                    deletes.append(i)
                    last_deleted = i
                    last_kept = i - 1
                    #print('first delete={} last_kept={}'.format(i, last_kept))                
            else:
                if self.is_collision_free(q_list[last_kept], q_list[i+1]): 
                    #print('last_kept={} delete={}'.format(last_kept, i))
                    #del q_list[i]                  
                    deletes.append(i)
                    last_deleted = i
                else:
                    last_kept = i
                    #print('keeping {}'.format(last_kept))
        
        for i in deletes[::-1]:
            del q_list[i]
          
        print('trimmed path length={}'.format(len(q_list)))
